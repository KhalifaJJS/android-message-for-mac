...
final class BackendManager: ObservableObject {
    ...
    var dataDir: String {
        // Folder named after THIS app (GoogleRCS) so macOS treats it as our own
        // data — accessing a differently-named folder ("OpenMessage") trips the
        // macOS "access another app's data" prompt on every launch.
        let direct = (NSHomeDirectory() as NSString).appendingPathComponent("Library/Application Support/GoogleRCS")
        if ensureDirectoryExists(at: direct) {
            return direct
        }
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let container = appSupport.appendingPathComponent("GoogleRCS").path
    ...
    func start() {
        ...
        proc.environment = [
            ...
            // Do NOT auto-import history on startup. The "deep" backfill calls
            // GetOrCreateConversation for every contact, which creates an empty
            // conversation thread with each one on Google Messages — alarming and
            // unwanted for a notifier. We only want to surface incoming messages,
            // so leave existing threads untouched and let live sync populate the
            // inbox. (History can still be pulled on demand from Settings.)
