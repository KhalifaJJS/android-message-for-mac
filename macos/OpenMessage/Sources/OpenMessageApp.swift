...
struct OpenMessageApp: App {
    ...
    @StateObject private var inbox: InboxMonitor
    ...
    init() {
        ...
        let inbox = InboxMonitor(baseURL: backend.baseURL)
        self._inbox = StateObject(wrappedValue: inbox)
        // Begin watching the backend immediately; the monitor retries until the
        // Go server is up, so it's safe to start before the backend finishes.
        inbox.start()
    }

    var body: some Scene {
        Window("Android Message for Mac", id: "main") {
            ContentView(backend: backend, notifications: notifications, contacts: contacts)
                .frame(minWidth: 800, minHeight: 500)
                .onOpenURL { url in
                    // Tapping the desktop widget opens openmessage://open — bring
                    // the inbox window to the front.
                    guard url.scheme == InboxShared.urlScheme else { return }
        ...
        MenuBarExtra {
            MenuBarView(backend: backend, inbox: inbox)
        } label: {
            MenuBarLabel(inbox: inbox)
        }
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate, @unchecked Sendable {
    func applicationDidFinishLaunching(_ notification: Notification) {
        // Auto-start at login so the menu bar dot and desktop widget keep
        // working without the user relaunching. Best-effort; the user can turn
        // this off under System Settings ▸ General ▸ Login Items.
...
private final class SettingsViewModel: ObservableObject {
    ...
    struct SIMOption: Identifiable, Hashable {
        let number: Int
        let label: String
        var id: Int { number }
    }
    @Published var sims: [SIMOption] = []
    @Published var defaultSim: Int = 0
    ...
    func loadSimSettings() async {
        if let (data, response) = try? await URLSession.shared.data(from: endpoint("api/sims")),
           let http = response as? HTTPURLResponse, http.statusCode == 200,
           let arr = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]] {
            sims = arr.compactMap { item in
                guard let n = (item["sim_number"] as? NSNumber)?.intValue else { return nil }
                let carrier = (item["carrier"] as? String) ?? ""
                let phone = (item["phone"] as? String) ?? ""
                var label = carrier.isEmpty ? "SIM \(n)" : carrier
                if !phone.isEmpty { label += " · \(phone)" }
                return SIMOption(number: n, label: label)
            }
        }
        if let (data, response) = try? await URLSession.shared.data(from: endpoint("api/settings/default-sim")),
           let http = response as? HTTPURLResponse, http.statusCode == 200,
           let obj = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
           let n = (obj["sim_number"] as? NSNumber)?.intValue {
            defaultSim = n
        }
    }

    func setDefaultSim(_ n: Int) async {
        do {
            var request = URLRequest(url: endpoint("api/settings/default-sim"))
            request.httpMethod = "POST"
            request.setValue("application/json", forHTTPHeaderField: "Content-Type")
            request.httpBody = try JSONSerialization.data(withJSONObject: ["sim_number": n])
            _ = try await URLSession.shared.data(for: request)
            defaultSim = n
            feedback = "기본 발신 SIM을 저장했습니다."
        } catch {
            feedback = "기본 SIM 저장에 실패했습니다."
        }
    }
                ...
                if googleNeedsPairing {
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        HStack(spacing: 10) {
                            ...
                            // Log out of the current Google account and start
                            // pairing a different one (QR / Google sign-in).
                            Button("계정 로그아웃 / 변경") {
                                openMainWindow()
                                backend.beginGooglePairing()
                            }
                        ...
                        // Dual-SIM: app-wide default outgoing SIM. Only shown when
                        // the phone reports more than one SIM.
                        if model.sims.count >= 2 {
                            Picker("기본 발신 SIM", selection: Binding(
                                get: { model.defaultSim },
                                set: { newValue in Task { await model.setDefaultSim(newValue) } }
                            )) {
                                Text("자동 (대화별)").tag(0)
                                ForEach(model.sims) { sim in
                                    Text(sim.label).tag(sim.number)
                                }
                            }
                            .pickerStyle(.menu)
