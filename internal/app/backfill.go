...
func (a *App) deepBackfill() {
	...
	// Phase C (DISABLED): contact-based discovery called GetOrCreateConversation
	// for every contact, which CREATES an empty conversation thread with each one
	// on Google Messages — it spams the user's phone with empty chats. We only
	// ever want to read EXISTING threads (Phases A/B above), never create new
	// ones, so this phase is intentionally skipped.
	a.Logger.Info().Msg("Deep backfill: skipping contact discovery (would create empty threads)")
...
// NOTE: contact-based conversation discovery (discoverFromContacts) and
// per-phone backfill (BackfillConversationByPhone) were removed entirely.
// They called GetOrCreateConversation, which CREATES an empty conversation
// thread on Google Messages for every contact — spamming the user's phone.
// This app never proactively creates threads.
