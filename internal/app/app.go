...
type App struct {
	...
	// Dual-SIM: the phone's SIM list, captured from the Settings event (there's
	// no on-demand fetch in the protocol).
	simMu sync.Mutex
	sims  []SIMRecord

	// RCS→SMS/MMS fallback: message IDs we've already auto-resent once, so a
...
func (a *App) LoadAndConnect() error {
	...
	// Sync the Google account's contacts (names + photos) so conversations show
	// real names/avatars instead of raw numbers. Best-effort, in the background.
	go func() {
		time.Sleep(3 * time.Second)
		if err := a.SyncGoogleContacts(); err != nil {
			a.Logger.Debug().Err(err).Msg("Contact sync failed")
		}
	}()
	return nil
}
