...
type Conversation struct {
	...
	// AvatarURL is populated on read from synced Google contacts (not stored).
...
func (s *Store) migrate() error {
	...
	-- Per-conversation preferred outgoing SIM (dual-SIM phones). 0 = use the
	-- conversation's default SIM. Kept in a side table so the main conversation
	-- columns/scans stay untouched.
	CREATE TABLE IF NOT EXISTS conversation_sim (
		conversation_id TEXT PRIMARY KEY,
		sim_number INTEGER NOT NULL DEFAULT 0
	);

	-- Simple app-wide key/value settings (e.g. the default outgoing SIM).
	...
	for _, col := range []string{
		...
		// Contact profile sync (Google account contacts → names + photos).
		"ALTER TABLE contacts ADD COLUMN number_key TEXT NOT NULL DEFAULT ''",
		"ALTER TABLE contacts ADD COLUMN photo TEXT NOT NULL DEFAULT ''",
	} {
		s.db.Exec(col) // ignore "duplicate column" errors
	}
	s.db.Exec(`CREATE INDEX IF NOT EXISTS idx_contacts_number_key ON contacts(number_key)`)
