...
func (s *Store) ListConversations(limit int) ([]*Conversation, error) {
	// Hide empty "tombstone" stubs (no name, no participants) — Google re-syncs
	// these after a thread is deleted and they'd otherwise show as "Unknown".
	rows, err := s.db.Query(`
		SELECT `+conversationColumns+`
		FROM conversations
		WHERE NOT (TRIM(name) = '' AND (participants = '[]' OR participants = ''))
