...
type EventHandler struct {
	...
	// OnSettings delivers the phone's settings (incl. the dual-SIM list) when it
...
func (h *EventHandler) handleMessage(evt *libgm.WrappedMessage) {
	...
	// Our own message came back failed → let the app decide whether to resend as
	// SMS/MMS (RCS fallback).
	if dbMsg.IsFromMe && !evt.IsOld && h.OnSendFailed != nil && strings.Contains(strings.ToUpper(dbMsg.Status), "FAILED") {
		h.OnSendFailed(dbMsg.ConversationID, dbMsg.MessageID, dbMsg.Status, dbMsg.Body)
	}
}
...
func (h *EventHandler) storeConversation(conv *gmproto.Conversation) bool {
	...
	// Skip empty tombstone conversations: after a user deletes a thread, Google
	// re-syncs a stub with no name and no participants. Storing it would show a
	// ghost "Unknown" conversation in the list.
	if strings.TrimSpace(conv.GetName()) == "" && participantsJSON == "[]" {
		h.Logger.Debug().Str("conv_id", conv.GetConversationID()).Msg("Skipping empty/tombstone conversation")
		return false
	}
